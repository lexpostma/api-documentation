...
You can set mode and permissions in one request. You cannot set `mode` to `to_full` and change `permissions` at the same time, as `permissions` is read-only in `to_full` mode. When mode is `predictive`, the permissions will be defined and continuously updated by the plan it receives from the cloud.
